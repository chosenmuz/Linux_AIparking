...
int callback(void *arg, int len, char **col_val, char **col_name)
{
    ...
    // 保存当前记录的时间的值
    if(i == 2)
        t = atol(col_val[i]); //用于将字符串转换为长整型数值
    }
    printf("\n");
    ...
    while(1)
    {
        ...
        else
        {
            ...
            snprintf(SQL, 100, "INSERT INTO info VALUES"
                            "('%x', '%s', '%lu');", id, license_plate, time(NULL)); //记录当前时间
...
{
    // 1，创建、打开一个数据库文件*.db
    int ret = sqlite3_open_v2("/mnt/AIParking/Imx6ull_code/build/parking.db", &db,
    ...
    sqlite3_exec(db, "CREATE TABLE IF NOT EXISTS info"     
             "(卡号 TEXT PRIMARY KEY, 车牌 TEXT, 时间 TEXT);",
                 NULL, NULL, &err);  //定义卡号为主键， 类型都为text  //
