...
int main(int argc, char* argv[])
{
	...
	*
	*/
	const char* session_begin_params = "engine_type = purextts,voice_name=xiaoyan, text_encoding = UTF8, tts_res_path = fo|res/tts/xiaoyan.jet;fo|res/tts/common.jet, sample_rate = 16000, speed = 50, volume = 50, pitch = 50, rdn = 2";
    ...
    addr.sin_addr.s_addr = inet_addr("192.168.5.11");
    ...
    while(1)
    {
        ...
        printf("收到【%s:%hu】待合成文本:%s\n",
                inet_ntoa(armAddr.sin_addr),        //网络地址转换成点分十进制的字符串
                ntohs(armAddr.sin_port), text);     //一个16位数由网络字节顺序转换为主机字节顺序
